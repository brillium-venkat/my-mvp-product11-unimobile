...
unimo_common_pre_reqs_validation() {
	...
	if [ -f "$output_tmp_file" ]; then
		delete_file=$(rm -rf "$output_tmp_file" 2>&1)
	fi
	...
	if [ $? -gt 0 ]; then
		...
		echo "$MOBILE_GENERATOR_LINE_PREFIX : Please set this path value - usually $HOME/Android/Sdk/emulator - and retry running this script."
...
if [ -f "$child1_output_tmp_file" ]; then
	delete_file=$(rm -rf "$child1_output_tmp_file" 2>&1)
fi

if [ -f "$emu_menu_opts" ]; then
	delete_file=$(rm -rf "$emu_menu_opts" 2>&1)
fi
...
if [[ "$server_rest_api_mode" == "https" ]]; then
	...
	echo "$MOBILE_GENERATOR_LINE_PREFIX : Starting Android emulator for running the Unimobile app on $target with http support..."
fi
